...
export function FileWorkspace({
  ...
  useEffect(() => {
    ...
    if (batch && batch.length > 0) {
      // The batch path takes the same ownership metadata as the single open:
      // a turn's artifact batch is an 'internal' activation, and classifying it
      // as the user's would let it retire its own settle watch.
      openFiles(batch, name, { origin, isStillOwned });
      return;
    }
